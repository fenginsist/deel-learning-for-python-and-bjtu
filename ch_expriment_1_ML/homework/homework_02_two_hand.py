...
# 二分类问题 一般采用 logistics 函数，采用二元交叉熵损失函数，梯度下降 做参数的优化
...
# 问题
# 1、debug 模型 时，X 是tensor，里面一层层的 H 和 T ，这是什么东西呢？困惑。毛师兄说是矩阵的转置，看了一下，好像是的
# 2、view() 函数统一为同型张量，这里有些疑问：本实验这里是为了变成行向量。
...
y1 = torch.zeros(100)  # 类型0 shape=(100)
x2 = torch.normal(-2 * n_data, 1)  # shape=(100, 2)
y2 = torch.ones(100)  # 类型1 shape=(100)

# 注意 x, y 数据的数据形式一定要像下面一样 (torch.cat 是合并数据)
train_features = torch.cat((x1, x2), 0).type(torch.FloatTensor)  # torch.Size([200, 2])
train_labels = torch.cat((y1, y2), 0).type(torch.FloatTensor)  # torch.Size([200])
