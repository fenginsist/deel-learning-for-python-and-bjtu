...
# 2、bug：TypeError: Optimizer.zero_grad() missing 1 required positional argument: 'self'
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.hidden_layer = nn.Sequential(
            nn.Linear(num_input, num_hiddens),  # 疑问
            nn.ReLU()
        )
        self.output_layer = nn.Linear(num_hiddens, num_output)
...
# 5)优化算法中的动量法
SGD_momentum_torch = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.5)
...
def train(net, train_iter, loss_func, num_epochs, lr=None, optimizer=None):
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_iter:
            ...
            l = loss_func(pre_y, y)
            # 梯度清零
            # 报错：Optimizer.zero_grad() missing 1 required positional argument: 'self'
            optimizer.zero_grad()
            l.backward()
            optimizer.step()
