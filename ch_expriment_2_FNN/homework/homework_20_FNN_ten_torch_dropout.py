...
model = MyTenNet(dropout=0)  # 模型 当 dropout 为0时，即什么也都没有操作
print('model:', model)
...
print('model:', model)
train_all_loss_2, test_all_loss_2, train_ACC_2, test_ACC_2 = train_and_test_torch(model=model_2, epochs=20, lr=0.01)
print('-----', train_all_loss_2, test_all_loss_2, train_ACC_2, test_ACC_2)

model_3 = MyTenNet(dropout=0.6)  # 模型
print('model:', model)
train_all_loss_3, test_all_loss_3, train_ACC_3, test_ACC_3 = train_and_test_torch(model=model_3, epochs=20, lr=0.01)
print('-----', train_all_loss_3, test_all_loss_3, train_ACC_3, test_ACC_3)

model_4 = MyTenNet(dropout=0.9)  # 模型
print('model:', model)
train_all_loss_4, test_all_loss_4, train_ACC_4, test_ACC_4 = train_and_test_torch(model=model_4, epochs=20, lr=0.01)
print('-----', train_all_loss_4, test_all_loss_4, train_ACC_4, test_ACC_4)

# 完成loss的显示
drop_name_1 = ['dropout=0', 'dropout=0.3', 'dropout=0.6', 'dropout=0.9', 'torch实现不同的dropout-Loss变化']
