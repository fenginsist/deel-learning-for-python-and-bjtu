...
print('model:', model)
train_all_loss, test_all_loss, train_ACC, test_ACC = train_and_test_torch(model=model, epochs=20, lr=0.01, weight_decay=0)
print('-----', train_all_loss, test_all_loss, train_ACC, test_ACC)


model_2 = MyTenNet(dropout=0)  # 模型
print('model:', model)
train_all_loss_2, test_all_loss_2, train_ACC_2, test_ACC_2 = train_and_test_torch(model=model_2, epochs=20, lr=0.01, weight_decay=1e-3)
